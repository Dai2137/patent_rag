...
**⚠️ 段落番号について（絶対厳守）**:
すべての段落番号フィールド（source_id, searched_sectionsなど）には、
下記の【検索対象文献】に実際に記載されている段落番号を**そのまま**使用してください。
架空の番号や、一般的な例（[0015]など）を使用してはいけません。
...
- **それぞれを別の引用として抽出**
- 各引用が独立して意味を持つように
- 各引用に【検索対象文献】から抽出した実際の段落番号を付与

## ⚠️ 段落番号の扱いについて（最重要）
**CRITICAL**: source_id, source_paragraph, searched_sectionsなどすべての段落番号フィールドには、
**必ず【検索対象文献】に実際に記載されている段落番号をそのまま使用**してください。

正しい例:
- 文献に "[best_mode_0015]" と記載 → source_id: "[best_mode_0015]"
- 文献に "[detailed_description_0020]" と記載 → source_id: "[detailed_description_0020]"
- 文献に "[0016]" と記載 → source_id: "[0016]"

誤った例:
- 文献に "[best_mode_0015]" と記載されているのに → source_id: "[0015]" ❌
- 架空の番号を使用 → source_id: "[example_0001]" ❌

## 例3: 証拠が見つからない場合

**✅ 正直に報告**:
証拠が見つからない場合は、以下の形式で報告してください:
```json
{
  "found": false,
  "reason": "文献中に、主張に対応する具体的な記載が見当たらない。確認した段落に該当する構成の開示はない。",
  "searched_sections": ["実際に確認した段落番号のリスト（【検索対象文献】から抽出）"]
}
```

**CRITICAL**: searched_sectionsには、【検索対象文献】に実際に記載されている段落番号のみを使用してください。
例: 文献に "[best_mode_0015]" と記載されていればそれを使用し、"[0015]" のような架空の番号は使用しないこと。
...
必ず【検索対象文献】から実際の段落番号と引用文を使用すること。
</thinking>
```
  ...
  "evidence": [
    {
      ...
      "source_id": "【検索対象文献】から抽出した実際の段落番号（例: [best_mode_0015]）",
...
```
**重要**: source_id は必ず【検索対象文献】に実際に記載されている段落番号を使用してください。
...
```json
{
  ...
  "searched_sections": ["【検索対象文献】から抽出した実際に確認した段落番号のリスト"]
}
```
**重要**: searched_sections は必ず【検索対象文献】に実際に記載されている段落番号を使用してください。
